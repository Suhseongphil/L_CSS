<!DOCTYPE html>
<%@page import="org.jsoup.helper.HttpConnection.Request"%>
...
<body>
	...
	<div class="container">
		<div class="row">
			...
			<%
			String category = request.getParameter("cfaddress");
			...
			<select
				style="margin-top: 25px; margin-right: 80px; padding: 8px; margin-left: 15px; font-weight: bold; font-size: 15px; float: right; height: 20%;"
				name="cfaddress" id="cfaddress" onchange="selectCheck()">

				<option id="check">선택</option>
				<option value="1">전체</option>
				<option value="인천" <%=category.equals("인천")|| category == null?"selected":""%>> 인천</option>
				<option value="서울" <%=category.equals("서울")|| category == null?"selected":""%>>서울</option>
				<option value="경기" <%=category.equals("경기")|| category == null?"selected":""%>>경기</option>
				<option value="충남" <%=category.equals("충남")|| category == null?"selected":""%>>충남</option>
				<option value="강원" <%=category.equals("강원")|| category == null?"selected":""%>>강원</option>
				<option value="경북" <%=category.equals("경북")|| category == null?"selected":""%>>경북</option>
				<option value="전남" <%=category.equals("전남")|| category == null?"selected":""%>>전남</option>
				<option value="전북" <%=category.equals("전북")|| category == null?"selected":""%>>전북</option>
			</select>
	...
	<div class="">
		<ul class="ul3" style="margin-bottom: 30px; margin-top: 30px;">
			<c:choose>
				...
				<c:otherwise>
					<li><a href="cafeList?page=${pagedto.page - 1}">«</a> <li>
